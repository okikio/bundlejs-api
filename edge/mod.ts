...
import { executePreparedBundle } from "./execute.ts";
import { prepareBundleRequest } from "./request.ts";
import { getPackageResultKey, headers } from "./constants.ts";

export type { Config } from "./types.ts";
export { headers } from "./constants.ts";
...
export default {
  async fetch(req: Request) {
    try {
      // Note: request parsing + esbuild execution is shared with Workers.
      // Storage/caching here remains Deno-specific (Upstash Redis + gists),
      // and we intentionally preserve the existing Deno Deploy behavior.
      ...
      const prepared = await prepareBundleRequest(url);
      const {
        badgeID,
        badgeKey,
        exportAll,
      ...
      const start = Date.now();
      const [response, resultText] = await executePreparedBundle(url, prepared);
