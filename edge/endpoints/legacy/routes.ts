...
export function registerLegacyRoutes<E extends Env = BlankEnv>(
  ...
  if (badgeSvg?.default) {
    app.get('/badge', ...(badgeSvg.Middleware ?? []), badgeSvg.default)
  }
  
  // GET /badge/raster, /badge-raster → /v1/badge/raster
  const badgeRaster = handlers['badge-raster']
  if (badgeRaster?.default) {
    app.get('/badge/raster', ...(badgeRaster.Middleware ?? []), badgeRaster.default)
    app.get('/badge-raster', ...(badgeRaster.Middleware ?? []), badgeRaster.default)
  }
  ...
  if (cacheClear?.default) {
    app.get('/clear-all-cache-123', ...(cacheClear.Middleware ?? []), cacheClear.default)
    app.get('/clear-cache', ...(cacheClear.Middleware ?? []), cacheClear.default)
  }
}