...
export type SupabaseClientSelectType = ReturnType<ReturnType<SupabaseClientType['from']>['select']>
...
/**
 * CursorData schema - the validated payload inside a signed cursor token.
 * 
 * This is what gets encoded into the cursor token and must be validated
 * both on encode (before signing) and decode (after signature verification).
 * 
 * sortValue accepts string | number | Date to handle various column types:
 * - string: timestamps, text columns
 * - number: numeric columns, integer IDs
 * - Date: JS Date objects (serialized to ISO string)
 */
...
/**
 * Cursor-based pagination normalized form
 * 
 * Note: decodedCursor is OPTIONAL because:
 * - During parsing/decoding: only the raw cursor string is available
 * - During validation (superRefine): decodedCursor gets populated after signature verification
 * - This matches the actual runtime behavior of the pagination flow
 */
