  ...
  const compressionMap = await (async (type?: CompressionType) => {
    switch (type) {
      case "brotli": {
        ...
        return async (code: Uint8Array<ArrayBuffer>) => await compress(code, code.length, quality ?? undefined);
      }
      case "zstd": {
        const { compress, getWASM } = await import("./deno/zstd/mod.ts");
        await getWASM();

        return async (code: Uint8Array<ArrayBuffer>) => await compress(code, quality ?? undefined);
      }
      case "lz4": {
        const { compress, getWASM } = await import("./deno/lz4/mod.ts");
        await getWASM();

        return async (code: Uint8Array<ArrayBuffer>) => await compress(code);
      }
      case "gzip":
      default: {
        const { compress } = await import("./deno/gzip/mod.ts");

        return async (code: Uint8Array<ArrayBuffer>) => {
