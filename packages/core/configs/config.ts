...
export function createConfig<T extends "transform" | "build" | "context", O extends TransformConfig | BuildConfig>(type: T, opts?: O) {
  if (type === "transform") {
    return deepMerge<TransformConfig>(structuredClone(TRANSFORM_CONFIG), opts as TransformConfig, { arrays: "replace" });
  } else {
    return deepMerge<BuildConfig>(structuredClone(BUILD_CONFIG), opts as BuildConfig, { arrays: "replace" });
  }
}