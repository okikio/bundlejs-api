...
export type ResultStorage = {
  readFile?: (value: BundleResult, resultText?: string) => Promise<string | Uint8Array | undefined>;
  setBadge?: (badgeKey: string, badgeID: string, badge: StoredBadge) => Promise<void>;
};
...
export async function generateResult([badgeKey, badgeID]: string[], [value, resultText]: [BundleResult, string | undefined], url: URL, cached: boolean, duration: number, storage?: ResultStorage) {
  ...
  if (badgeQuery) {
    ...
    try {
      await storage?.setBadge?.(badgeKey, badgeID, {
    ...
    return new Response(typeof imgShield === "string" ? imgShield : toArrayBuffer(imgShield), {
      status: 200,
      headers: [
        ...headers,
  ...
  if (fileQuery) {
    const { fileId } = value;
    let fileResult = await storage?.readFile?.(value, resultText);

    if (fileResult === undefined && fileId) {
      const { getFile } = await import("./gist.ts");
      fileResult = await getFile(fileId);
    }

    if (fileResult === undefined && resultText !== undefined) {
      fileResult = resultText;
    }
    ...
    return new Response(typeof fileResult === "string" ? fileResult : toArrayBuffer(fileResult), {
      status: 200,
      headers: [
        ...headers,
