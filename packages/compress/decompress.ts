...
export async function decompress(inputs: Uint8Array[] | string[] = [], opts: CompressConfig = {} as CompressionOptions) {
  ...
  const compressionMap = await (async () => {
    switch (type) {
      case "brotli": {
        ...
        return async (code: BufferSource) => await decompress(code, (code as Uint8Array).length);
      }
      case "zstd": {
        const { decompress, getWASM } = await import("./deno/zstd/mod.ts");
        await getWASM();

        return async (code: BufferSource) => await decompress(code);
      }
      case "lz4": {
      ...
      default: {
        ...
        return async (code: BufferSource) => {
          if (!('DecompressionStream' in globalThis)) {
            return await Promise.resolve(code as Uint8Array<ArrayBuffer>)
          }
  ...
  const compressedContent = await Promise.all(
    contents.map((content) => compressionMap(content as BufferSource))
  );
