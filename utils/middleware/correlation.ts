...
/**
 * Returns a LogTape logger that is automatically scoped to the current service.
 * 
 * This keeps handler code short while preserving the category structure used by
 * the repo's correlation and logging setup. Callers can request either a single
 * extra category or a full category suffix, and the service prefix is applied
 * for them.
 * 
 * @param c - Hono context with correlation middleware already applied
 * @param categories - Optional category suffix to append after the service name
 * @returns Logger instance scoped to the current service request flow
 * 
 * @example Use the default service logger
 * ```ts
 * const logger = getLogger(c)
 * logger.info('Processing request')
 * ```
 * 
 * @example Add a narrower category inside a handler
 * ```ts
 * const logger = getLogger(c, ['auth', 'session'])
 * logger.warn('Session missing')
 * ```
 */
