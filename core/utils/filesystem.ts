/// <reference lib="dom" />
import type {
  FileSystemFileHandle as TypeFileSystemFileHandle,
  FileSystemFileHandleWithPath,
  FileSystemDirectoryHandle as IFileSystemDirectoryHandle
} from "./types.ts";
import { decode, encode } from "@bundle/utils/encode-decode";
import { dirname, basename, resolve, posix } from "@bundle/utils/path";
...
export async function getFile<T, F extends IFileSystem<T, Content>, Content = Uint8Array>(fs: F, path: string, type: string = "buffer", importer?: string): Promise<string | Awaited<Content> | null> {
  ...
  try {
    ...
    if (type === "string" && ArrayBuffer.isView(file)) return decode(file as BufferSource);
    return file;
 ...
 */
async function writeFile(fileHandle: FileSystemFileHandle | TypeFileSystemFileHandle, contents: Uint8Array | string) {
  ...
  if ("createSyncAccessHandle" in fileHandle && fileHandle.createSyncAccessHandle) {
    ...
    try {
      ...
      accessHandle.write(encodedMessage as BufferSource, { at: 0 });
  ...
  await writable.write(contents as BufferSource);
 ...
 */
async function readFile(fileHandle: FileSystemFileHandle | TypeFileSystemFileHandle) {
  // https://developer.mozilla.org/en-US/docs/Web/API/FileSystemFileHandle/createSyncAccessHandle
  if ("createSyncAccessHandle" in fileHandle && fileHandle.createSyncAccessHandle) {
    // Get sync access handle
    const accessHandle = await fileHandle.createSyncAccessHandle();
