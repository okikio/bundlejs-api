...
import type { CorrelationVariables } from '@utils/middleware/correlation'
...
import { ok, internalServerError, badRequest, validationFailed } from '@utils/response'
import { getLogger } from '@utils/middleware/correlation'
...
export const Handler: EndpointHandler<AppEnv, typeof Definition> = async function (c) {
  ...
  try {
    ...
    const resolution = await resolveBundleRequest({
      ...
      baseUrl: c.req.url,
      cacheMode,
      includeOutputText: false, // isLegacyPath ? 'auto' : 
    })
